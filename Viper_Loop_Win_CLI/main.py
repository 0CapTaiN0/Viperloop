...
def set_dns(network_card, dns_server1, dns_server2):
    try:
        subprocess.call(f'netsh interface ip set dns "{network_card}" static {dns_server1}', shell=True)
        subprocess.call(f'netsh interface ip add dns "{network_card}" {dns_server2} index=2', shell=True)
...
def reset_dns(network_card):
    try:
        subprocess.call(f'netsh interface ipv4 set dnsservers "{network_card}" source=dhcp', shell=True)
    ...
    while True:
        try:
            ...
            if choice.isdigit() and 0 <= int(choice) <= len(options):
                return options[int(choice) - 1][1] if int(choice) != 0 else "Exit"
            else:
...
if __name__ == "__main__":
    ...
    if dns_action == "Set DNS":
        ...
        selected_network_card = prompt_user([(i, option) for i, option in enumerate(active_cards, start=1)], "Select a network card:")
